...
export function matchesBudgetSearch(category: string, subcategory: string, query: string) {
  const normalize = (value: string) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLocaleLowerCase("it");
  return normalize(`${category} ${subcategory}`).includes(normalize(query.trim()));
}
