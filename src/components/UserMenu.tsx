...
export default function UserMenu() {
  ...
  return <details className="relative min-w-0"><summary className={buttonClasses({ variant: "ghost", size: "sm", className: "max-w-40 sm:max-w-60" })}><UserRound size={17} className="shrink-0" aria-hidden /><span className="truncate">{user.name || user.email || "Profilo"}</span></summary><div className="absolute right-0 z-50 mt-2 min-w-56 rounded-xl border bg-white p-2 shadow-lg"><p className="truncate px-3 py-2 text-xs text-muted-fg">{user.email}</p><Link className="block rounded px-3 py-2 hover:bg-black/5" href={`/${locale}/profilo`}>Profilo</Link><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left hover:bg-black/5" onClick={async () => { const sb = getBrowserClient(); const { data } = await sb.auth.getSession(); const jwt = data.session?.access_token; if (jwt) await fetch("/api/my/current-event", { method: "DELETE", headers: { Authorization: `Bearer ${jwt}` } }); localStorage.removeItem("eventType"); localStorage.removeItem("currentEventChangedAt"); await sb.auth.signOut(); window.location.assign(`/${locale}`); }}><LogOut size={16} aria-hidden />Logout</button></div></details>;
}
