...
export default function UserMenu() {
  ...
  return <details className="relative"><summary className={buttonClasses({ variant: "ghost", size: "sm" })}><UserRound size={17} aria-hidden />{user.name || user.email || "Profilo"}</summary><div className="absolute right-0 z-50 mt-2 min-w-56 rounded-xl border bg-white p-2 shadow-lg"><p className="truncate px-3 py-2 text-xs text-muted-fg">{user.email}</p><Link className="block rounded px-3 py-2 hover:bg-black/5" href={`/${locale}/profilo`}>Profilo</Link><button className="flex w-full items-center gap-2 rounded px-3 py-2 text-left hover:bg-black/5" onClick={async () => { await getBrowserClient().auth.signOut(); window.location.assign(`/${locale}`); }}><LogOut size={16} aria-hidden />Logout</button></div></details>;
}
