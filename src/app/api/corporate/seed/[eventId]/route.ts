...
export async function POST(
  ...
  const template = getCorporateTemplate(country);

  // Get corporate event type
  ...
  if (!typeRow) {
    return NextResponse.json({ ok: false, error: "Corporate event type not found" }, { status: 404 });
  }
  ...
  return NextResponse.json({ ok: true, message: "Corporate event seeded successfully" });
}
