...
export type ExpenseRow = {
  ...
  supplier?: string;
  amount?: number;
  spendType?: "common"; // Single budget for graduation
  notes?: string;
};
...
export async function GET(req: NextRequest) {
  ...
  // Find graduation event for user
  ...
  // Fetch categories/subcategories for graduation
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "graduation").single();
