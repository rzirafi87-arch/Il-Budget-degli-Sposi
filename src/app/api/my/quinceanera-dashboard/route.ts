...
export async function GET(req: NextRequest) {
  ...
  if (!jwt) {
    ...
    const tpl = getQuinceaneraTemplate(country);
  ...
  // Find quinceanera event for user
  ...
  // Fetch categories/subcategories for quinceanera
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "quinceanera").single();
  ...
  for (const e of expenses || []) {
    ...
    if (row) {
      ...
      row.spendType = "common"; // Quinceañera is family event (single-budget)
    }
  }
    ...
    await db.from("expenses").upsert(
      {
        ...
        spend_type: "common", // Force common for quinceanera (family event)
      },
      { onConflict: "event_id,subcategory_id" }
    );
  }

  return NextResponse.json({ ok: true, message: "Quinceañera data saved" });
}
