...
import { getAnniversaryTemplate } from "@/data/templates/anniversary";
...
export async function GET(req: NextRequest) {
  ...
  if (!jwt) {
    ...
    const tpl = getAnniversaryTemplate(country);
  ...
  // Find anniversary event for user
  ...
  // Fetch categories/subcategories for anniversary
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "anniversary").single();
