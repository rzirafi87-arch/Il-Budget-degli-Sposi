...
export async function GET(req: NextRequest) {
  ...
  if (!jwt) {
    ...
    const tpl = getGenderRevealTemplate(country);
  ...
  // Find gender-reveal event for user
  ...
  // Fetch categories/subcategories for gender-reveal
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "gender-reveal").single();
