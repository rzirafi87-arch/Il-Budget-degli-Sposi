...
// Returns all categories and subcategories for confirmation event type
export async function GET(req: NextRequest) {
  try {
    ...
    if (!jwt) {
      ...
      const template = getConfirmationTemplate(country);
    ...
    const { data: eventType } = await db
      .from("event_types")
      .select("id")
      .eq("slug", "confirmation")
      .single();

    if (!eventType) {
      return NextResponse.json(
        { error: "Confirmation event type not configured" },
        { status: 500 }
      );
    }
    ...
    if (!categories || categories.length === 0) {
      return NextResponse.json(
        { error: "No categories found for confirmation" },
        { status: 500 }
      );
    }
    ...
    categories.forEach((cat) => {
      ...
      subs.forEach((sub) => {
        ...
        rows.push({
          ...
          spendType: "common", // Confirmation always common
          notes: expense?.notes || "",
        });
      });
    });
    ...
    return NextResponse.json({
      event: {
        id: eventData.id,
        name: eventData.name || "La mia Cresima",
    ...
    console.error("/api/my/confirmation-dashboard error", e);
    return NextResponse.json(
...
// POST /api/my/confirmation-dashboard
// Save confirmation budget and expenses
  ...
  try {
    ...
    if (!eventData) {
      ...
      const { data: newEvent, error: createError } = await db
        ...
        .insert({
          user_id: userId,
          name: event?.name || "La mia Cresima",
    ...
    if (categories && Array.isArray(categories)) {
      for (const row of categories) {
        ...
        const expenseData = {
          ...
          amount: row.amount || 0,
          spend_type: "common", // Always common for confirmation
          notes: row.notes || "",
          status: "approved",
          from_budget: true,
        };
    ...
    return NextResponse.json({
      success: true,
      message: "Confirmation dashboard saved successfully",
      eventId,
    });
  } catch (e) {
    console.error("/api/my/confirmation-dashboard POST error", e);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 }
    );
