...
export type ExpenseRow = {
  ...
  supplier?: string;
  amount?: number;
  spendType?: "common"; // Single budget for communion
  notes?: string;
};
...
export async function GET(req: NextRequest) {
  ...
  // Fetch categories/subcategories for communion
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "communion").single();
