...
import { getProposalTemplate } from "@/data/templates/proposal";
...
export async function GET(req: NextRequest) {
  ...
  if (!jwt) {
    ...
    const tpl = getProposalTemplate(country);
  ...
  // Fetch categories/subcategories for proposal
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "proposal").single();
  ...
  for (const e of expenses || []) {
    ...
    if (row) {
      ...
      row.spendType = "common"; // Proposal is always single-budget
    }
  }
    ...
    await db.from("expenses").upsert(
      {
        ...
        spend_type: "common", // Force common for proposal (single-budget event)
      },
      { onConflict: "event_id,subcategory_id" }
    );
  }

  return NextResponse.json({ ok: true, message: "Proposal data saved" });
}
