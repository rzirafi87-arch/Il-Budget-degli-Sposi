...
import { getCorporateTemplate } from "@/data/templates/corporate";
...
export async function GET(req: NextRequest) {
  ...
  if (!jwt) {
    ...
    const tpl = getCorporateTemplate(country);
  ...
  // Fetch categories/subcategories for corporate
  const { data: typeRow } = await db.from("event_types").select("id").eq("slug", "corporate").single();
  ...
  for (const e of expenses || []) {
    ...
    if (row) {
      ...
      row.spendType = "common"; // Corporate is always single-budget (company pays)
    }
  }
    ...
    await db.from("expenses").upsert(
      {
        ...
        spend_type: "common", // Force common for corporate (company-paid event)
      },
      { onConflict: "event_id,subcategory_id" }
    );
  }

  return NextResponse.json({ ok: true, message: "Corporate data saved" });
}
