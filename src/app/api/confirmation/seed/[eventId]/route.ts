 ...
 * Seed categories and subcategories for a confirmation (Cresima) event
 */
export async function POST(
  ...
  try {
    ...
    if (!eventType) {
      return NextResponse.json(
        { error: "Confirmation event type not found in database" },
        { status: 500 }
      );
    }

    // Get categories for confirmation
    ...
    return NextResponse.json({
      success: true,
      message: `Seeded ${insertedCount} subcategories for confirmation event`,
      eventId,
      country,
    });
  } catch (e) {
    console.error("/api/confirmation/seed/[eventId] error:", e);
    return NextResponse.json(
