...
export async function GET(request: NextRequest) {
  const limit = checkPublicRateLimit(request, "locations");
  if (!limit.allowed) return rateLimitResponse(limit.resetAt);
  try {
    const parsed = parseCatalogSearch(new URL(request.url).searchParams, "location");
    const response = await searchCatalog(parsed);
    return NextResponse.json({ locations: response.results.map((item) => ({ ...item.details, id: item.id, name: item.name, city: item.city, province: item.province, region: item.region, country_code: item.country, latitude: item.latitude, longitude: item.longitude, venue_type: item.category, verification_status: item.verificationStatus, confidence_score: item.confidenceScore, distance_km: item.distanceKm })), pagination: { ...response.pagination, limit: response.pagination.pageSize } }, { headers: { "Cache-Control": PUBLIC_CATALOG_CACHE } });
  } catch (error) {
