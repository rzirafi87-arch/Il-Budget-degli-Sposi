...
export async function POST(
  ...
  const template = getBarMitzvahTemplate(country);

  // Get bar-mitzvah event type
  ...
  if (!typeRow) {
    return NextResponse.json({ ok: false, error: "Bar Mitzvah event type not found" }, { status: 404 });
  }
  ...
  return NextResponse.json({ ok: true, message: "Bar Mitzvah event seeded successfully" });
}
