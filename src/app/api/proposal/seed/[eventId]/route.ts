...
export async function POST(
  ...
  const template = getProposalTemplate(country);

  // Get proposal event type
  ...
  if (!typeRow) {
    return NextResponse.json({ ok: false, error: "Proposal event type not found" }, { status: 404 });
  }
  ...
  return NextResponse.json({ ok: true, message: "Proposal event seeded successfully" });
}
