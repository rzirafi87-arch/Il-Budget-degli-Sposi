...
export default function BeautyPage() {
  ...
  params.append("category", "Make-up & Beauty");
      ...
      console.error("Errore caricamento beauty:", e);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  async function addToFavorites(beautyId: string, beautyName: string) {
    try {
      const { data } = await supabase.auth.getSession();
      const jwt = data.session?.access_token;
      ...
      if (res.ok) {
        showToast(`${beautyName} aggiunto ai preferiti!`, "success");
      } else {
  ...
  return (
    <section className="pt-6">
      ...
      <nav className="mb-4 flex items-center gap-2 text-sm text-gray-600">
        ...
        <span>›</span>
        <span className="text-gray-900 font-medium">Make-up & Beauty</span>
      </nav>

      {/* Header con bottone Torna a Fornitori */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h2 className="font-serif text-3xl mb-2">💄 Make-up & Beauty</h2>
          <p className="text-gray-600 text-sm sm:text-base leading-relaxed">
            Make-up artist, hair stylist ed estetica per il tuo giorno speciale.
          </p>
        </div>
        <Link
          href="/fornitori"
      ...
      <div className="mb-6 p-6 rounded-2xl border border-gray-200 bg-white/70 shadow-sm">
        <h3 className="font-semibold mb-4">Filtra professionisti beauty</h3>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Nome professionista o studio..."
              />
