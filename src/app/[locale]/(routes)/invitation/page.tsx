"use client";
import { invitationResumePath, safeInternalPath } from "@/lib/auth";
...
export default function InvitationPage() {
  ...
  async function decide(action:"accept"|"reject") { setBusy(true); setError(null); const {data}=await getBrowserClient().auth.getSession(); if(!data.session){const saved=await fetch("/api/invitations/return",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token,locale})}); const result=await saved.json().catch(()=>({})); if(!saved.ok){setBusy(false);setError(result.error||"INVITATION_INVALID");return;} const resume=safeInternalPath(result.next,invitationResumePath(locale)); window.location.assign(`/${locale}/auth?next=${encodeURIComponent(resume)}`);return;} const r=await fetch(`/api/invitations/${action}`,{method:"POST",headers:{Authorization:`Bearer ${data.session.access_token}`,"Content-Type":"application/json"},body:JSON.stringify({token})}); const j=await r.json(); setBusy(false); if(!r.ok){setError(j.error);return;} window.location.replace(action==="accept"?`/${locale}/dashboard`:`/${locale}/select-event`); }
