...
export default function SuppliersPage() {
  ...
  async function toggle(id:string){setError(null);const current=savedBy(id);const {data}=await getBrowserClient().auth.getSession();const token=data.session?.access_token;if(!token){setError(ui("authRequired"));return;}const r=await fetch(current?`/api/my/suppliers?resource_id=${current.id}`:"/api/my/suppliers",{method:current?"DELETE":"POST",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:current?undefined:JSON.stringify({supplier_id:id})});if(r.ok){if(current)setSaved(v=>v.filter(s=>s.id!==current.id));else{const j=await r.json();setSaved(v=>[...v,j.savedSupplier]);}}else setError(ui("saveError"));}
  async function toggleSelection(id:string){const current=savedBy(id);if(!current)return;setPendingId(id);setError(null);try{const {data}=await getBrowserClient().auth.getSession();const token=data.session?.access_token;if(!token){setError(ui("authRequired"));return;}const status=current.status==="SELECTED"?"SAVED":"SELECTED";const response=await fetch("/api/my/suppliers",{method:"PATCH",headers:{"content-type":"application/json",authorization:`Bearer ${token}`},body:JSON.stringify({resource_id:current.id,status})});if(!response.ok){setError(planningT("supplier.updateError"));return;}const body=await response.json();setSaved(value=>value.map(item=>item.id===current.id?body.savedSupplier:item));}finally{setPendingId(null);}}
