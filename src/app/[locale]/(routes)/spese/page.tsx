...
function ExpensesPage() {
  ...
  // Funzione per aprire il modal storico
  ...
  // Carica lo storico modifiche per una spesa
  const loadExpenseHistory = async (expenseId: string) => {
    ...
    <section className="pt-6">
      ...
      <div className="mb-6 flex flex-wrap gap-2 items-end">
        <select className="border rounded px-2 py-1" value={filterCategory} onChange={e => setFilterCategory(e.target.value)}>
          ...
          {/* Qui puoi aggiungere le opzioni dinamiche se vuoi */}
        </select>
      </div>
