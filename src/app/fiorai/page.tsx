...
export default function FioraiPage() {
  const { showToast } = useToast();
  const [items, setItems] = useState<Florist[]>([]);
  ...
  params.append("category", "Fiorai");
      ...
      console.error("Errore caricamento fiorai:", e);
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  async function addToFavorites(floristId: string, floristName: string) {
    try {
      const { data } = await supabase.auth.getSession();
      const jwt = data.session?.access_token;
      ...
      if (res.ok) {
        showToast(`${floristName} aggiunto ai preferiti!`, "success");
      } else {
  ...
  return (
    <section className="pt-6">
      ...
      <nav className="mb-4 flex items-center gap-2 text-sm text-gray-600">
        ...
        <span>›</span>
        <span className="text-gray-900 font-medium">Fiorai</span>
      </nav>

      {/* Header con bottone Torna a Fornitori */}
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1">
          <h2 className="font-serif text-3xl mb-2">💐 Fiorai</h2>
          <p className="text-gray-600 text-sm sm:text-base leading-relaxed">
            Allestimenti floreali, bouquet, centrotavola e decorazioni per la cerimonia e il ricevimento.
          </p>
        </div>
        <Link
          href="/fornitori"
      ...
      <ImageCarousel images={PAGE_IMAGES.fiorai || PAGE_IMAGES.fornitori} height="280px" />

      {/* Filtri */}
      <div className="mb-6 p-6 rounded-2xl border border-gray-200 bg-white/70 shadow-sm">
        <h3 className="font-semibold mb-4">Filtra fiorai</h3>
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder="Nome negozio o fiorista..."
              />
      ...
      <div className="mb-6 flex justify-between items-center">
        <p className="text-sm text-gray-600">
          {filtered.length} {filtered.length === 1 ? "fiorista trovato" : "fioristi trovati"}
        </p>
        <div className="flex gap-2">
          <button
      ...
      {loading ? (
        ...
        <div className="p-10 text-center text-gray-400 rounded-2xl border border-gray-200 bg-white/70">
          ...
          <p className="text-lg font-medium">Nessun fiorista trovato</p>
