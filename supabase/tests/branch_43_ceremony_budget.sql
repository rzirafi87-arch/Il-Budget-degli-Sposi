...
select has_column('public','budget_items','source','budget item origin exists');
...
insert into public.budget_items(event_id,country_code,name,amount)
values('43000000-0000-4000-8000-000000000001','it','Voce manuale sintetica',100);
...
select is((select source from public.budget_items where name='Voce manuale sintetica'),'manual','existing and manually inserted budget rows default to manual');
