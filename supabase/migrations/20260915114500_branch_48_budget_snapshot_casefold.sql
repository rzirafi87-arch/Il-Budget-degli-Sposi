...
begin
  ...
  for item in select value from jsonb_array_elements(coalesce(p_rows,'[]'::jsonb)) loop
    ...
    select id into v_category_id from public.categories where event_id=p_event_id and lower(name)=lower(category_name) order by id limit 1;
    ...
    select id into v_subcategory_id from public.subcategories where category_id=v_category_id and (canonical_key=item_key or (item_key is null and lower(name)=lower(subcategory_name))) order by id limit 1;
