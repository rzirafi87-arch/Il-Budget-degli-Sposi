...
// Categorie e sottocategorie predefinite per matrimoni
...
export default function DashboardPage() {
  ...
  // Genera tutte le righe base dalle categorie e sottocategorie
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Se ci sono dati salvati, usali, altrimenti genera tutte le righe
  ...
  // Budget diviso tra sposo e sposa (50% ciascuno)
  ...
  const remainingCommon = totalBudget - totalCommon - totalBride - totalGroom;
    ...
    <section className="pt-6">
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="p-4 rounded-lg border border-pink-200 bg-pink-50">
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-semibold">€ {formatEuro(budgetBride)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Speso:</span>
                <span className="font-semibold text-pink-600">€ {formatEuro(totalBride)}</span>
              </div>
              <div className="flex justify-between border-t pt-1">
                <span className="text-gray-700">Residuo:</span>
                <span className={`font-bold ${remainingBride < 0 ? "text-red-600" : "text-green-600"}`}>
                  € {formatEuro(remainingBride)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg border border-blue-200 bg-blue-50">
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Disponibile:</span>
                <span className="font-semibold">€ {formatEuro(budgetGroom)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Speso:</span>
                <span className="font-semibold text-blue-600">€ {formatEuro(totalGroom)}</span>
              </div>
              <div className="flex justify-between border-t pt-1">
                <span className="text-gray-700">Residuo:</span>
                <span className={`font-bold ${remainingGroom < 0 ? "text-red-600" : "text-green-600"}`}>
                  € {formatEuro(remainingGroom)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-4 rounded-lg border border-gray-300 bg-gray-50">
            ...
            <div className="space-y-1 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-600">Disponibile:</span>
                <span className="font-semibold">€ {formatEuro(totalBudget)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-gray-600">Speso:</span>
                <span className="font-semibold text-gray-700">€ {formatEuro(totalSpent)}</span>
              </div>
              <div className="flex justify-between text-xs text-gray-500">
                <span>- Comune:</span>
                <span>€ {formatEuro(totalCommon)}</span>
              </div>
              <div className="flex justify-between border-t pt-1">
                <span className="text-gray-700">Residuo:</span>
            ...
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-gray-50 hover:bg-gray-50/60">
                ...
                <td className="px-2 py-2">
                  <input
                    type="number"
                    className="border border-gray-200 rounded px-2 py-1 w-full text-right text-xs"
                    value={row.amount || ""}
                ...
                <td className="px-2 py-2">
                  <select
                    ...
                    value={row.spendType}
                    onChange={(e) => updateRow(row.id, "spendType", e.target.value)}
                  >
      ...
      <div className="mt-8">
        <button
          onClick={saveData}
          disabled={saving}
          className="bg-[#A3B59D] text-white rounded-lg px-6 py-3 hover:bg-[#8a9d84] font-medium disabled:opacity-50 disabled:cursor-not-allowed"
        >
