...
type Totals = { total: number; common: number; bride: number; groom: number };
type Row = {
  category: string;
  subcategory: string;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Carica le spese approvate
        const expensesRes = await fetch("/api/my/expenses", { headers });
        const expensesData = await expensesRes.json();
        
        // Filtra solo le spese approvate
        const approvedExpenses = (expensesData.expenses || []).filter(
          (exp: any) => exp.status === "approved"
        );

        // Converti le spese in righe del budget
        ...
        // Ordina alfabeticamente per categoria e sottocategoria
        budgetRows.sort((a, b) => {
